...
/*!
 * brief Disable slots configuration
 *
 * Disable all slots related to context passed as parameter.
 *
 * param base asrc base pointer.
 * param context index of asrc context, reference asrc_context_t.
 */
void ASRC_DisableContextSlots(ASRC_Type *base,
                              asrc_context_t context)
{
    uint32_t i;

    for (i = 0; i < ASRC_SUPPORT_MAXIMUM_CONTEXT_PROCESSOR_NUMBER; i++) {
        if (ASRC_GET_SLOT0_CONTEXT_INDEX(i) == context)
            base->PROC_CTRL_SLOT0_R0[i] = 0;        // Disable SLOT0 and clean R0 register
        if (ASRC_GET_SLOT1_CONTEXT_INDEX(i) == context)
            base->PROC_CTRL_SLOT1_R0[i] = 0;        // Disable SLOT1 and clean R0 register
    }
}
    ...
    for (i = 0U; i < ASRC_SUPPORT_MAXIMUM_CONTEXT_PROCESSOR_NUMBER; i++)
    {
        ...
        if (availableSlot == 0U)
        {
            if (config->filterSt2Taps != 0U)
            {
                ...
                stage1MemSize = (config->filterSt1Taps - 1U) * config->filterSt1Exp * availableChannel + availableChannel;
                stage1MemAddr = 0U;
...
/*!
 * brief Return resampler fractional bits based on context
 *
 * param config ASRC context configuration pointer.
 *
 * retval fractional bits of resampler configuration
 */
uint32_t ASRC_GetResampleFractBits(asrc_context_config_t *config)
{
    assert(config != NULL);

    uint32_t fract_bits;

    switch (config->contextResampler.tap) {
    case kASRC_ResamplerTaps_32:    /*!< resampler taps 32 */
        fract_bits = 39;    // ratio coded as 5.39 format
        break;
    case kASRC_ResamplerTaps_64:    /*!< resampler taps 64 */
        fract_bits = 38;    // ratio coded as 6.38 format
        break;
    default:                        /*!< resampler taps 128 */
        fract_bits = 37;    // ratio coded as 7.37 format
        break;
    }
...
/*!
 * brief ASRC update data type from context configuration.
 *
 * param config ASRC context configuration pointer.
 * param inDataType input data type.
 * param outDataType output data type.
 */
void ASRC_Update_dataType_Config(asrc_context_config_t *config,
                                 asrc_data_type_t inDataType,
                                 asrc_data_type_t outDataType)
{
    assert(config != NULL);

    config->contextInput.dataFormat.dataType    = inDataType;
    config->contextOutput.dataFormat.dataType   = outDataType;
}
                                  ...
                                  uint32_t outSampleRate)
{
    ...
    config->contextInput.dataFormat.dataPosition     = 0;
    config->contextInput.dataFormat.dataEndianness   = kASRC_DataEndianLittle;
    config->contextInput.dataFormat.dataWidth        = kASRC_DataWidth32Bit;
    ...
    config->contextOutput.dataFormat.dataWidth        = kASRC_DataWidth32Bit;
...
status_t ASRC_SetContextConfig(ASRC_Type *base, asrc_context_t context, asrc_context_config_t *config)
{
    ...
    if ((config->contextChannelNums == 0) ||
        (config->contextChannelNums > ASRC_SUPPORT_MAXIMUM_CHANNEL_NUMBER))
    {
        return kStatus_InvalidArgument;
    }
 ...
 * param inWidth input samples buffer size, the size of buffer should be converted to align with 4 bytes.
    ...
    ASRC_Type *base, asrc_context_t context, uint32_t *outAddr, uint32_t outWidth, uint32_t sampleCount)
{
    ...
    for (i = 0U; i < sizeToRead; i++)
    {
        *outAddr = ASRC_ReadContextFifo(base, context);
        outAddr = (uint32_t *) ((uint8_t *)outAddr + outWidth);
    }
...
/*!
 * brief Discard the ASRC read fifo remained samples.
 * Discard any sample remaining in ASRC read fifo
 *
 * param base ASRC base pointer.
 * param context context id.
 * retval sample counts actual read from output fifo.
 */
uint32_t ASRC_DiscardFIFORemainedSample(ASRC_Type *base, asrc_context_t context)
{
    uint32_t remainSample = base->SAMPLE_FIFO_STATUS[context] & ASRC_SAMPLE_FIFO_STATUS_NUM_SAMPLE_GROUPS_OUT_MASK;
    uint32_t channel      = (base->CTX_CTRL[context] & ASRC_CTX_CTRL_NUM_CH_EN_MASK) + 1U;
    uint32_t i, sizeToRead;

    sizeToRead = remainSample * channel;

    if (sizeToRead == 0U)
        return 0U;

    for (i = 0U; i < sizeToRead; i++)
        (void) ASRC_ReadContextFifo(base, context);
 ...
 * ASRC_SetContextConfig.
 * The data format it supports:
 *  1.16bit 16bit per sample in input buffer, input buffer size should be calculate as: samples * 2U, output buffer
 *    size can be calculated by call function ASRC_GetContextOutSampleSize with the outWidth parameter set to 2
 *  2.20bit 24bit per sample in input buffer, input buffer size should be calculate as: samples * 3U, output buffer
 *    size can be calculated by call function ASRC_GetContextOutSampleSize with the outWidth parameter set to 3.
 *  3.24bit 24bit per sample in input buffer, input buffer size should be calculate as: samples * 3U, output buffer
 *    size can be calculated by call function ASRC_GetContextOutSampleSize with the outWidth parameter set to 3.
 *  4.32bit 32bit per sample in input buffer, input buffer size should be calculate as: samples * 4U, output buffer
 *    size can be calculated by call function ASRC_GetContextOutSampleSize with the outWidth parameter set to 4.
 *
 * param base ASRC base pointer.
 * param context context id.
...
status_t ASRC_TransferBlocking(ASRC_Type *base, asrc_context_t context, asrc_transfer_t *xfer)
{
    ...
    uint32_t outWidth =
        ((base->CTX_OUT_CTRL[context] & ASRC_CTX_OUT_CTRL_BITS_PER_SAMPLE_MASK) >> ASRC_CTX_OUT_CTRL_BITS_PER_SAMPLE_SHIFT);
    uint32_t inMask =
    ...
    inWidth =
        (inWidth == (uint32_t)kASRC_DataWidth32Bit ?
            4U :
            inWidth == (uint32_t)kASRC_DataWidth16Bit ? 2U : 3U);       // 3 for 20 and 24 bits
    outWidth =
        (outWidth == (uint32_t)kASRC_DataWidth32Bit ?
            4U :
            outWidth == (uint32_t)kASRC_DataWidth16Bit ? 2U : 3U);      // 3 for 20 and 24 bits
    ...
    uint32_t writtenSamples, readSamples, i;
    ...
    while (inSamples != 0U)
    {
        ...
        if ((ASRC_GetFifoStatus(base, context) & (uint32_t)kASRC_FifoStatusInputFifoWatermarkFlag) != 0U)
        {
            for (i = 0U; i < writtenSamples; i++)
            {
                ...
                writePointer = (uint32_t *) ((uint8_t *)writePointer + inWidth);
            }
            inSamples -= writtenSamples;
        }

        while ((ASRC_GetFifoStatus(base, context) & (uint32_t)kASRC_FifoStatusOutputFifoWatermarkFlag) != 0U)
        {
            for (i = 0U; i < readSamples; i++)
            {
                *readPointer = ASRC_ReadContextFifo(base, context);
                readPointer = (uint32_t *) ((uint8_t *)readPointer + outWidth);
            }
    ...
    xfer->outDataSize = outSamples * outWidth;

    return kStatus_Success;
}

/*!
 * brief ASRC blocking push to convert audio sample rate.
 * This function depends on the configuration of input, so it should be called after the
 * ASRC_SetContextConfig.
 * The data format it supports:
 *  1.16bit 16bit per sample in input buffer, input buffer size should be calculate as: samples * 2U
 *  2.20bit 24bit per sample in input buffer, input buffer size should be calculate as: samples * 3U
 *  3.24bit 24bit per sample in input buffer, input buffer size should be calculate as: samples * 3U
 *  4.32bit 32bit per sample in input buffer, input buffer size should be calculate as: samples * 4U
 *
 * param base ASRC base pointer.
 * param context context id.
 * param xfer transfer configuration.
 * retval kStatus_Success.
 */
status_t ASRC_PushBlocking(ASRC_Type *base, asrc_context_t context, asrc_transfer_t *xfer)
{
    assert((xfer->inDataAddr != NULL) && (xfer->outDataAddr != NULL));
    assert(xfer->inDataSize % sizeof(uint32_t) == 0U);
    ...
    while (inSamples != 0U)
    {
        /* Wait for free space in input FIFO */
        while (0 == (ASRC_GetFifoStatus(base, context) & (uint32_t)kASRC_FifoStatusInputFifoWatermarkFlag));
...
#if 0
// Following ASRC_TransferBlocking function is the one from MCUXpresso
/*!
 * brief ASRC blocking convert audio sample rate.
 * This function depends on the configuration of input and output, so it should be called after the
 * ASRC_SetContextConfig.
...
/*!
 * @brief Dumps ASRC register.
 *
 * @param base ASRC base pointer.
 */
#define DUMP_LEVEL  LK_VERBOSE
void ASRC_Dump(ASRC_Type *base)
{
    uint32_t i;

    printlk(DUMP_LEVEL, "ASRC base address: %p\n", base);
    for (i = 0; i < ASRC_SUPPORT_MAXIMUM_CONTEXT_PROCESSOR_NUMBER; i++) {
        printlk(DUMP_LEVEL, "\tCTX_CTRL_%d:\t\t%#08x\n", i, base->CTX_CTRL[i]);
        printlk(DUMP_LEVEL, "\tCTX_CTRL_EXT1%d:\t\t%#08x\n", i, base->CTX_CTRL_EXT1[i]);
        printlk(DUMP_LEVEL, "\tCTX_CTRL_EXT2%d:\t\t%#08x\n", i, base->CTX_CTRL_EXT2[i]);
        printlk(DUMP_LEVEL, "\tCTRL_IN_ACCESS%d:\t%#08x\n", i, base->CTRL_IN_ACCESS[i]);

        printlk(DUMP_LEVEL, "\tCTX_OUT_CTRL%d:\t\t%#08x\n", i, base->CTX_OUT_CTRL[i]);
        printlk(DUMP_LEVEL, "\tCTRL_OUT_ACCESS%d:\t%#08x\n", i, base->CTRL_OUT_ACCESS[i]);
    }
}
