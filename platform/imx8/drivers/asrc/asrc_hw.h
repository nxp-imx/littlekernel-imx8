...
#define FSL_ASRC_DRIVER_VERSION (MAKE_VERSION(2, 0, 2)) /*!< Version 2.0.2 */
 // The code is coming from MCUXpresso v2.8 with some API and code updates
/*@}*/
...
typedef struct _asrc_context_prefilter_config
{
    ...
    /*
     * application can untouch below configuration by set filterCoeffAddress to NULL, driver will load it from the
...
typedef struct _asrc_context_resampler_config
{
    ...
    asrc_resampler_taps_t tap;              /*!< resampler taps */
    /*
     * application can untouch below configuration by set filterCoeffAddress to NULL, driver will load it from the
     * driver firmware table, otherwise driver will use the value directly.
     */
 ...
 * param context index of asrc context, reference asrc_context_t.
 */
void ASRC_DisableContextSlots(ASRC_Type *base, asrc_context_t context);
...
/*!
 * @brief write the ASRC Resampling Ratio Update Control.
 *
 * @param base ASRC base pointer.
 * @param context context id.
 * @param ratioUpdate ratio update to write.
 */
static inline void ASRC_SetResamplingRatioModifier(ASRC_Type *base, asrc_context_t context, int32_t ratioUpdate)
{
    base->RS_UPDATE_CTRL[context] = ratioUpdate;
}
